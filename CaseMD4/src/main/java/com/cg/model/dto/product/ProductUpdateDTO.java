...
public class ProductUpdateDTO implements Validator {
    ...
    public void validate(Object target, Errors errors) {
        ...
        if (price != null && price.length() > 0) {
            ...
            if (price.length() < 3){
                errors.rejectValue("price", "price.min","Giá sản phẩm thấp nhất là 100 $");
                return;
            }
        ...
        if (quantity != null && quantity.length() > 0) {
            ...
            if (!quantity.matches("(^$|[0-9]*$)")){
                errors.rejectValue("quantity",  "quantity.number","Số lượng sản phẩm phải là số ,vui lòng nhập giá sản phẩm.");
                return;
            }
